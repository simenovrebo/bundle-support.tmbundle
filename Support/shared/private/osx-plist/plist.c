...
CFNumberRef convertNumber(VALUE obj) {
	// The values live until CFNumberCreate() has read them (they were block-scoped, which modern compilers reuse)
	double doubleValue;
	long long longValue;
	switch (TYPE(obj)) {
		case T_FLOAT:
			doubleValue = NUM2DBL(obj);
			return CFNumberCreate(kCFAllocatorDefault, kCFNumberDoubleType, &doubleValue);
		case T_FIXNUM: // a Fixnum can be larger than an int on 64-bit systems
		case T_BIGNUM:
			longValue = NUM2LL(obj);
			return CFNumberCreate(kCFAllocatorDefault, kCFNumberLongLongType, &longValue);
		default:
